...
import FeatureGuideMenu from './FeatureGuideMenu.vue'
...
async function saveTitle(conversationId) {
  ...
  if (nextTitle && nextTitle !== currentConversation?.title) {
    await chatStore.updateConversation(conversationId, nextTitle)
  }
  ...
  <aside class="sidebar" :class="{ 'is-collapsed': isCollapsed }">
    ...
    <div class="new-chat-wrap">
      <button class="new-chat-btn" @click="handleNewChat" title="새 채팅">
        ...
        <span v-if="!isCollapsed">새 채팅</span>
      </button>
    </div>
    ...
    <div class="scroll" v-if="!isCollapsed">
      ...
      <div class="chat-list">
        <div
          ...
          class="chat-item"
          :class="{
            'is-active': chat.id === chatStore.activeConversationId,
            'is-editing': editingId === chat.id
          }"
    ...
    <!-- 기능 가이드 -->
    <div class="guide-wrap" v-if="!isCollapsed">
      <FeatureGuideMenu />
    </div>

    <!-- 접힌 상태 안내 버튼 -->
    <div class="collapsed-guide-wrap" v-else>
      <button
        class="collapsed-guide-button"
        type="button"
        title="사이드바를 펼치면 기능 가이드를 볼 수 있어요"
        @click="toggleSidebar"
      >
        💡
      </button>
...
.sidebar.is-collapsed .new-chat-wrap {
  padding: 4px 12px 12px;
}
...
.chat-item.is-active .chat-item-title {
  font-weight: 600;
  color: #18181B;
}
...
.chat-item-edit-input:focus {
  border-color: #3B6EF5;
  box-shadow: 0 0 0 2px rgba(59, 110, 245, 0.1);
}

.chat-item-edit-actions {
  display: flex;
  align-items: center;
  gap: 4px;
}
...
.chat-item.is-active .chat-item-meta {
  color: #9A9AA2;
}
...
.sidebar.is-collapsed .user-footer {
  justify-content: center;
  padding: 11px 0;
}
